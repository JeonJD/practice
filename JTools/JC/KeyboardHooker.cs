...
namespace JC
{
    public class KeyboardHooker
    {
        ...
        private delegate int HookedKeyboardEventHandler(int nCode, int wParam, IntPtr lParam);
        ...
        /// <returns></returns>
        public delegate int HookedKeyboardUserEventHandler(int iKeyWhatHappened, int vkCode);
...
//        public static IntPtr m_CurrentWindowHWnd;
        ...
        private static extern void CopyMemory(ref KBDLLHOOKSTRUCT pDest, IntPtr pSource, int cb);
        ...
        private static int HookedKeyboardProc(int nCode, int wParam, IntPtr lParam)
        {
            int lResult = 0;

            if (nCode == HC_ACTION) //LowLevelKeyboardProc
            {
                //빌드 옵션에서 안전하지 않은 코드 허용에 체크
                unsafe
                {
                ...
                {
                    if ( //Civ클라이언트 혹은 JC가 활성화 되었을 때만 후킹
                        GetForegroundWindow() == (IntPtr)(FindWindow("CryENGINE", null)) ||
                        GetForegroundWindow() == (IntPtr)(FindWindow(null, "JC")) ||
                        GetForegroundWindow() == (IntPtr)(FindWindow(null, "DoItDoIt"))
                       )
                    {
